...
# ============================================================
#  FLUX RADIO — ExoFM Réunion
# ============================================================
STATIONS = [
    {
        "name": "EXO FM",
        "url": "https://exofmreunion.ice.infomaniak.ch/exofmreunion-64.aac",
        "type": "audio"
    },
    {
        "name": "EXO FM TV",
        "url": "https://eu-west-3-antenne-reunion-msl.akamaized.net/hls/live/20000128/main/ch1.m3u8",
        "type": "video"
    },
]

SITE_LINKS = {
    "site":      ("ExoFM.re",           "https://exofm.re/"),
    "tv":        ("ExoFM TV",           "https://exofm.re/tv/"),
    "replay":    ("Replay & Podcasts",  "https://exofm.re/replay/"),
    "radios":    ("Webradios",          "https://exofm.re/radios/"),
    "app":       ("Application ExoFM",  "https://exofm.re/app/"),
}
...
class MPVVideoWidget(QOpenGLWidget):
    ...
    def _get_process_address(self, _ctx, name):
        """Callback pour récupérer les adresses des fonctions OpenGL.
        Signature : (ctx, name) -> int
        """
        glctx = QOpenGLContext.currentContext()
        if glctx is None:
            return 0
        # name est déjà un bytes -> QByteArray
    ...
    def initializeGL(self):
        """Initialise mpv et le contexte de rendu OpenGL."""
        self.mpv = mpv.MPV(
            vo='libmpv',
            ao='pulse',
            ytdl=False,
    ...
    def _on_mpv_update(self):
        """Callback appelé par mpv quand une nouvelle frame est prête."""
        self.update()

    def paintGL(self):
        """Rendu de la frame mpv dans le framebuffer OpenGL du widget."""
...
class SitePopup(QDialog):
    def __init__(self, parent=None):
        ...
        grid = QGridLayout()
        grid.setSpacing(6)
        items = [("site", "Site"), ("tv", "ExoFM TV"), ("replay", "Replay"),
                 ("radios", "Webradios"), ("app", "Application")]
    ...
    def _open_all(self):
        for key in ("site", "tv", "replay", "radios", "app"):
            if key in SITE_LINKS:
                _, url = SITE_LINKS[key]
                webbrowser.open(url)
        self.accept()
...
# ================= FENÊTRE PRINCIPALE =================
class ExoFM(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("RADIO EXOFM")
        self.resize(520, 480)
    ...
    def showEvent(self, event):
        ...
        if self.audio_player is None:
            self.audio_player = mpv.MPV(
                ...
                audio_client_name='ExoFM', force_window='no', idle='yes',
            )
    ...
    def toggle_pause(self):
        if self._current_type == "video":
            ...
            self.lbl_meta.setText("PAUSE" if paused else "TV EN DIRECT")
            return
